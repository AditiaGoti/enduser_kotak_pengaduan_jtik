<template>
  ...
  <div v-if ="isLoggedIn">
    <div v-if="profileuser.email == null">
...
<button type="button" class="ml-auto -mx-1.5 -my-1.5 bg-red-50 text-red-500 rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8 dark:bg-gray-800 dark:text-red-400 dark:hover:bg-gray-700" data-dismiss-target="#alert-2" aria-label="Close" @click="closeAlert">
  ...
  <div class="text-left mb-5 bg-blue-800 rounded shadow py-6">
    <div class="md:ml-36 ml-0 px-5 md:px-0">
<h1 class="mb-4 text-2xl font-extrabold text-white md:text-2xl lg:text-4xl"><span class="text-transparent bg-clip-text bg-gradient-to-r to-white from-sky-400 mr-2">KOTAK PENGADUAN</span>JTIK</h1>
<p class="text-sm font-normal text-white lg:text-lg md:w-[500px] w-fit">Dengan Kotak Pengaduan JTIK, pengaduan Anda dapat langsung dibaca dan ditanggapi.</p>
            <div v-if="!isLoggedIn"> 
          ...
          <div v-else>
            ...
            <button
              class="bg-white hidden text-blue-600 active:bg-blue-800 text-xs font-bold uppercase px-4 py-2 my-3 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 mb-3 ease-linear transition-all duration-150"
              type="button"
            >
              <i class="fas fa-arrow-alt-circle-down"></i> Buat Pengaduan
            </button>
  ...
  <p class="font-bold md:text-2xl  text-sm mb-2 underline decoration-2 text-left md:ml-32 ml-2 "> Pengaduan Populer </p>
